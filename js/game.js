...
function saveGame() {
  const data = {
    ...
    money: G.money, flags: G.flags, seen: G.seen, caught: G.caught,
...
function loadGame() {
  try {
    ...
    Object.assign(G, {
      ...
      bag: data.bag, money: data.money, flags: data.flags,
...
function interact() {
  ...
  let npc = (map.npcs || []).find((n) => !npcHidden(n) && n.x === tx && n.y === ty);
  // talk across counters
  if (!npc && tileAt(map, tx, ty) === "C") {
    npc = (map.npcs || []).find((n) => !npcHidden(n) && n.x === tx + dx && n.y === ty + dy);
  }
  ...
  else if (t === "M") showDialog(["Complicated machinery. Better not touch it."]);
  else if (t === "~") showDialog(["The water is calm and clear."]);
}
...
function endBattle() {
  ...
  const wasLeader = b.trainer && b.trainer.isLeader;
  ...
  if (res === "win" && b.mode === "trainer" && trainerKey) {
    ...
    if (wasLeader) {
      showDialog([
        "SLATE: You have real talent, kid.",
        G.playerName + " received the QUARRY BADGE!",
        "SLATE: That badge proves you beat the OAKRIDGE GYM.",
        "SLATE: You've cleared this adventure — but keep training your team, champ!",
        "Thanks for playing MONSTERQUEST!",
      ], { onDone: () => { G.flags.badge = true; processPending(); } });
      return;
    }
  }
  processPending();
...
function drawOverworld() {
  ...
  for (const npc of map.npcs || []) {
    if (npcHidden(npc)) continue;
    chars.push({ x: npc.x * TILE, y: npc.y * TILE, kind: npc.sprite, dir: npc.dir, bob: 0 });
  }
  ...
  for (const c of chars) {
    const spr = personSprite(c.kind, c.dir, 2);
...
function drawTrainerCard() {
  drawUnderlay();
  drawBox(70, 60, 340, 240);
  drawText("TRAINER CARD", 90, 74, "#202030", 18);
  ctx.drawImage(personSprite("player", "down", 4), 320, 90);
  drawText("NAME", 90, 120, "#606070", 14);
  drawText(G.playerName, 180, 120);
  drawText("MONEY", 90, 150, "#606070", 14);
  drawText("$" + G.money, 180, 150);
  const ids = Object.keys(SPECIES);
  drawText("DEX", 90, 180, "#606070", 14);
  drawText(`${ids.filter((id) => G.caught[id]).length} caught`, 180, 180);
  drawText("BADGES", 90, 210, "#606070", 14);
  if (G.flags.badge) {
    ctx.fillStyle = "#b8a038";
    ctx.beginPath(); ctx.arc(192, 220, 10, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#e8d878";
    ctx.beginPath(); ctx.arc(192, 220, 6, 0, Math.PI * 2); ctx.fill();
    drawText("QUARRY BADGE", 214, 212, "#202030", 14);
  } else {
    drawText("none yet", 180, 210);
  }
  drawText("Z/X: back", 90, 268, "#606070", 13);
}
