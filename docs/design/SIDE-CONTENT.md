...
## 3. COMPANION CATS: MEADOW & BIGBOY

- **Following**: one cat walks behind you (swap at any Care centre or by whistling in the pause menu). MEADOW moves fast and darts ahead when idle; BIGBOY lumbers and sits when you stop.
- **Sniffing**: an idle animation + a paw-print emote when within 3 tiles of a hidden item, ripe ingredient, or a Bounty clue. Tap the cat to reveal. MEADOW smells *items/ingredients*, BIGBOY smells *creatures/bounties* (his prompt reveals encounter species chance for the tile).
...
- **Trust levels (0–5, each cat)**: earned by feeding (Cat's Cup, fish), winning with the cat in the active slot, choosing kind quest options, daily gifts. Perks: T1 sniff range 5; T2 cat carries a held item; T3 MEADOW: dodges the first hit of a battle once per fight; BIGBOY: *Back from the Brink* also heals 25%; T4 cat can be sent to auto-fish/gather while you're elsewhere (real-time timers); T5 cat can accompany you *inside* gyms and the Arena, and learns a signature *regular* move (MEADOW: *Skitter* — priority +1, steals 1 spe stage; BIGBOY: *Big Sit* — ends foe's stat stages, Def +2). Their Overdrive signatures (SYSTEMS-SPEC §7) are separate: MEADOW *Zoomies*, BIGBOY *Brink Roar*. BIGBOY's *Back from the Brink* gains the ally shield at Y Berllan (`bigboy_shield`, story).
- Battle: cats are party members with fixed species but grow like others; they never go to the box, and can be set to "reserve" without leaving.
...
| 4 The Dish Goes Dark | Holmes Chapel, Jodrell gate, Congleton | 17–22 | 12–16 | Q6, Q10, Q11 | SIGNAL METER, CUTOVER counter, rematches, BIGBOY Shove, Gritstone Grips (optional) | Badge BEAR, Ghost Lens |
...
| 6 Brine and Perry | Nantwich, Y Berllan | 25–30 | 20–24 | Q17, Q18, Q30 (opens) | brewing tier 1–2, BIGBOY shield, Hack Green key | Badge TOKEN, Clarifier |
